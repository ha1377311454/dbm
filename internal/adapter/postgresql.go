...
func (a *PostgreSQLAdapter) buildDSN(config *model.ConnectionConfig) string {
	var params []string

	// host=localhost port=5432 user=postgres password= dbname= sslmode=disable
...
func (a *PostgreSQLAdapter) Update(db *sql.DB, database, table string, data map[string]interface{}, where string) error {
	...
	for strings.Contains(whereClause, "?") {
		placeholderKey := fmt.Sprintf("$%d", argNum)
		placeholderMap["?"] = placeholderKey
		whereClause = strings.Replace(whereClause, "?", placeholderKey, 1)
		argNum++
	}
	values = append(values, whereClause)
...
func (a *PostgreSQLAdapter) buildAddColumnSQL(database, table string, col *model.ColumnDef) (string, error) {
	if col == nil {
...
// modifyColumn 修改列（PostgreSQL 需要多个语句）
func (a *PostgreSQLAdapter) modifyColumn(db *sql.DB, database, table string, col *model.ColumnDef) error {
	if col == nil {
		return fmt.Errorf("column definition is required")
	}
...
func (a *PostgreSQLAdapter) buildColumnType(col *model.ColumnDef) string {
	var parts []string
...
// getBaseType 获取基础类型
func (a *PostgreSQLAdapter) getBaseType(col *model.ColumnDef) string {
...
func (a *PostgreSQLAdapter) formatDefaultValue(value string) string {
...
func (a *PostgreSQLAdapter) buildAddIndexSQL(database, table string, idx *model.IndexDef) (string, error) {
	if idx == nil {
